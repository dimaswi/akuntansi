...
export default function Show({ department, stocks, summary, filters }: Props) {
    ...
    const handleSearch = () => {
        router.get(
            route('department-stocks.show', department.id),
            {
                search: searchTerm,
                    ...
                    <CardHeader>
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <CardDescription>Summary of stock inventory for this department</CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-2xl font-bold text-green-600">{formatNumber(summary.total_quantity)}</p>
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Item Stok Rendah</p>
                                <p className={`mt-1 text-2xl font-bold ${summary.low_stock_items > 0 ? 'text-red-600' : 'text-gray-900'}`}>
                                    {summary.low_stock_items}
                                </p>
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Total Value</p>
                                <p className="mt-1 text-2xl font-bold text-blue-600">{formatCurrency(summary.total_value)}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardHeader>
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <CardDescription>Total {stocks.total} item</CardDescription>
                            </div>
                                    ...
                                    {stocks.data.length === 0 ? (
                                        ...
                                        stocks.data.map((stock) => (
                                            <TableRow key={stock.id} className="hover:bg-muted/50">
                                                <TableCell className="font-mono text-xs text-muted-foreground">{stock.item.code}</TableCell>
                                                <TableCell className="font-medium">{stock.item.name}</TableCell>
                                                <TableCell className="text-sm text-muted-foreground">{stock.item.category?.name || '-'}</TableCell>
                                                <TableCell className="text-center">
                                                    <Badge variant="outline" className="text-xs">
                                                        {stock.item.unit_of_measure}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell className="text-right font-semibold">{formatNumber(stock.quantity_on_hand)}</TableCell>
                                                <TableCell className="text-right text-sm">{formatCurrency(stock.average_unit_cost)}</TableCell>
                                                <TableCell className="text-right font-semibold text-blue-600">
                                                    {formatCurrency(stock.total_value)}
                                                </TableCell>
                                                <TableCell className="text-sm text-muted-foreground">{formatDate(stock.last_updated_at)}</TableCell>
                                            </TableRow>
                                        ))
                                    )}
                                </TableBody>
                        ...
                        {stocks.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <div className="text-sm text-muted-foreground">
                                    ...
                                    {Math.min(stocks.current_page * stocks.per_page, stocks.total)} of {stocks.total} entries
                                </div>
                                <div className="flex gap-2">
                                    <Button
                                        variant="outline"
