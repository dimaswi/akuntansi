...
interface Props {
    ...
    suppliers?: Supplier[];
    isLogistics?: boolean; // Add this to know if user is from logistics
}
            ...
            <div className="max-w-7xl p-4 sm:px-6 lg:px-8">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">
                                    {isLogistics 
                                        ? 'Kelola data items inventory farmasi dan umum untuk semua department (Role: Logistics)'
                                        : 'Items yang tersedia untuk department Anda'
                                    }
                                </p>
                            </div>
                        </div>
                ...
                <Card className="mb-6">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                            ...
                            {/* Show Department filter only for logistics users */}
                            {isLogistics && (
                                <div className="space-y-2">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Total {items?.meta?.total || 0} items ditemukan
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {!items?.data || items.data.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <p className="mt-1 text-sm text-gray-500">
                                    {filters?.search 
                                        ? 'Tidak ada items yang sesuai dengan kriteria pencarian.'
                                        : 'Belum ada items yang tersedia untuk department Anda. Hubungi logistics untuk penambahan stock.'
                                    }
                                </p>
                                {isLogistics && (
                                    <div className="mt-6">
                            ...
                            <>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                {isLogistics && <TableHead>Department</TableHead>}
                                                <TableHead>Satuan</TableHead>
                                                <TableHead>Stok Current</TableHead>
                                        ...
                                        <TableBody>
                                            {items?.data?.map((item) => {
                                                ...
                                                const departmentStock = item.department_stocks?.[0];
                                                const currentStock = departmentStock?.current_stock || 0;
                                                const minStock = departmentStock?.minimum_stock || item.reorder_level;
                                                const maxStock = departmentStock?.maximum_stock || item.max_level;
                                                ...
                                                return (
                                                    <TableRow key={item.id}>
                                                        ...
                                                        {isLogistics && (
                                                            <TableCell>
                                                                {departmentStock?.department?.name || item.department?.name || '-'}
                                                            </TableCell>
                                                        )}
                                ...
                                {items?.meta?.last_page && items.meta.last_page > 1 && (
                                    <div className="flex items-center justify-between space-x-2 py-4">
                                        <div className="text-sm text-muted-foreground">
                                            Menampilkan {((items?.meta?.current_page || 1) - 1) * (items?.meta?.per_page || 15) + 1} sampai{' '}
                                            {Math.min((items?.meta?.current_page || 1) * (items?.meta?.per_page || 15), items?.meta?.total || 0)} dari{' '}
                                            {items?.meta?.total || 0} items
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => handlePageChange((items?.meta?.current_page || 1) - 1)}
                                                disabled={(items?.meta?.current_page || 1) <= 1}
                                            >
                                                Sebelumnya
                                            </Button>
                                            <div className="flex items-center space-x-1">
                                                {Array.from({ length: Math.min(5, items?.meta?.last_page || 1) }, (_, i) => {
                                                    const page = i + Math.max(1, (items?.meta?.current_page || 1) - 2);
                                                    if (page > (items?.meta?.last_page || 1)) return null;
                                                    return (
                                                        <Button
                                                            key={page}
                                                            variant={page === (items?.meta?.current_page || 1) ? "default" : "outline"}
                                                            size="sm"
                                            ...
                                            <Button
                                                ...
                                                size="sm"
                                                onClick={() => handlePageChange((items?.meta?.current_page || 1) + 1)}
                                                disabled={(items?.meta?.current_page || 1) >= (items?.meta?.last_page || 1)}
                                            >
                                                Selanjutnya
                                            </Button>
                                        </div>
