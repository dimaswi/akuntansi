...
export default function PostToJurnal({ giroTransaction, daftarAkun, nomorJurnalPreview }: Props) {
    // Handle undefined/null giroTransaction early
    if (!giroTransaction) {
        return (
            <AppLayout>
    ...
    const { data, setData, post, processing, errors } = useForm({
        giro_transaction_id: giroTransaction.id,
    ...
    // Get giro account COA
    const giroAccount = giroTransaction.giro_account.daftar_akun;

    // Store original amount for validation
    const originalAmount = giroTransaction.jumlah;
    const isPenerimaan = ['terima_giro'].includes(giroTransaction.jenis_transaksi);

    // Initialize with first row (giro account - now editable)
    useEffect(() => {
        if (!giroAccount) {
            console.error('Giro account not found in transaction data');
            return;
        }

        const firstRow: DetailJurnalRow = {
            id: 'giro-auto',
            daftar_akun_id: giroAccount.id.toString(),
            keterangan: giroTransaction.keterangan,
            jumlah_debit: isPenerimaan ? giroTransaction.jumlah : 0,
            jumlah_kredit: isPenerimaan ? 0 : giroTransaction.jumlah,
    ...
    const addRow = () => {
        const newRow: DetailJurnalRow = {
            id: `new-row-${Date.now()}`,
            daftar_akun_id: '',
            keterangan: giroTransaction.keterangan, // Use transaction keterangan as default
            jumlah_debit: 0,
            jumlah_kredit: 0,
            is_readonly: false
        };
    ...
    const updateRow = (id: string, field: keyof DetailJurnalRow, value: any) => {
        setDetailRows(detailRows.map(row => {
            ...
            // Validate if this is the giro row (first row) and amount exceeds original
            if (row.id === 'giro-auto') {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('kas.giro-transactions.post-to-journal'));
    };

    const getJenisColor = (jenis: string) => {
        const colors: Record<string, { bg: string; text: string; border: string }> = {
            'terima_giro': { bg: 'bg-green-50 dark:bg-green-900/20', text: 'text-green-700 dark:text-green-300', border: 'border-green-200 dark:border-green-700' },
            'tolak_giro': { bg: 'bg-red-50 dark:bg-red-900/20', text: 'text-red-700 dark:text-red-300', border: 'border-red-200 dark:border-red-700' },
            'cair_giro': { bg: 'bg-blue-50 dark:bg-blue-900/20', text: 'text-blue-700 dark:text-blue-300', border: 'border-blue-200 dark:border-blue-700' },
        };
    ...
    const jenisColor = getJenisColor(giroTransaction.jenis_transaksi);

    // Show loading if data not ready
    if (!giroTransaction || !giroAccount) {
        return (
            <AppLayout>
                <Head title="Post Transaksi Giro ke Jurnal" />
                <div className="p-4 sm:px-6 lg:px-8">
            ...
            <div className="p-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-6">
                    <div className="flex items-center space-x-4 mb-2">
                        <Link href={route('kas.giro-transactions.index')}>
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Cross-check transaksi giro ke jurnal akuntansi</p>
                        </div>
                    </div>
                </div>
                        ...
                        <Card className="border-gray-200 dark:border-gray-700 bg-gradient-to-br from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 sticky top-4">
                            <CardHeader className="pb-3 border-b border-gray-200 dark:border-gray-700">
                                <CardTitle className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                                    <FileCheck2 className="h-4 w-4 mr-2 text-gray-600 dark:text-gray-400" />
                                    Info Transaksi Giro
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="pt-4 space-y-3">
                                <div>
                                    ...
                                    <p className="text-xs font-mono font-semibold text-gray-900 dark:text-gray-100">{giroTransaction.nomor_transaksi}</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center text-xs text-gray-700 dark:text-gray-300">
                                        ...
                                        {formatDate(giroTransaction.tanggal_transaksi)}
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <Badge className={`text-[10px] px-2 py-0.5 h-5 font-medium ${jenisColor.text} ${jenisColor.bg} border ${jenisColor.border}`}>
                                        {giroTransaction.jenis_transaksi.replace(/_/g, ' ').toUpperCase()}
                                    </Badge>
                                </div>

                                <div>
                                    <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Data Giro</p>
                                    <p className="text-xs font-semibold text-gray-900 dark:text-gray-100">{giroTransaction.giro_account.nama_penerbit}</p>
                                    <p className="text-[10px] font-mono text-gray-500 dark:text-gray-400 mt-0.5">No. Giro: {giroTransaction.giro_account.nomor_giro}</p>
                                </div>

                                <div>
                                    <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Jumlah</p>
                                    <p className="text-lg font-bold font-mono text-gray-900 dark:text-gray-100">{formatCurrency(giroTransaction.jumlah)}</p>
                                </div>

                                <div>
                                    <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Pihak Terkait</p>
                                    <p className="text-xs text-gray-700 dark:text-gray-300">{giroTransaction.pihak_terkait}</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center text-xs text-gray-700 dark:text-gray-300">
                                        ...
                                        {giroTransaction.user?.name || 'Unknown'}
                                    </div>
                                </div>
                                ...
                                <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                                    ...
                                    <p className="text-xs text-gray-700 dark:text-gray-300 italic">{giroTransaction.keterangan}</p>
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <Card className="border-gray-200 dark:border-gray-700 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
                                ...
                                <CardContent className="pt-4 space-y-4">
                                                ...
                                                <table className="w-full text-xs">
                                                    ...
                                                    <tbody>
                                                        {detailRows.map((row, index) => {
                                                            // Check if Giro row exceeds original amount
                                                            const GiroExceeded = row.id === 'giro-auto' && (
                                                                ...
                                                                <td className="py-2 px-3">
                                                                    {row.id === 'giro-auto' ? (
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            <div>
                                                                                <p className="text-xs font-semibold text-gray-900 dark:text-gray-100">{giroTransaction.giro_account.nama_penerbit}</p>
                                                                                <p className="text-[10px] font-mono text-gray-500 dark:text-gray-400">No. {giroTransaction.giro_account.nomor_giro}</p>
                                                                            </div>
                                                                        </div>
                                                                    ) : (
                                    ...
                                    {/* Giro Amount Warning */}
                                    {(() => {
                                        const GiroRow = detailRows.find(r => r.id === 'giro-auto');
                                        const GiroExceeded = GiroRow && (
                                            (isPenerimaan && GiroRow.jumlah_debit > originalAmount) ||
                                            (!isPenerimaan && GiroRow.jumlah_kredit > originalAmount)
                                        );
                                                ...
                                                <div className="p-3 rounded-lg border bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700">
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span className="text-xs font-semibold text-amber-700 dark:text-amber-300">
                                                            Peringatan: Jumlah Giro melebihi jumlah transaksi original ({formatCurrency(originalAmount)})
                                                        </span>
                                                    </div>
                                                </div>
                                            );
