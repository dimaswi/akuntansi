...
interface Category {
    ...
    account_expense_id: number | null;
    is_active: boolean;
}
...
export default function EditCategory() {
    const { category, accounts } = usePage<Props>().props;
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        account_asset_id: category.account_asset_id?.toString() ?? '',
        account_depreciation_id: category.account_depreciation_id?.toString() ?? '',
        account_expense_id: category.account_expense_id?.toString() ?? '',
        is_active: category.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('aset.categories.update', category.id), {
            onError: () => toast.error('Gagal memperbarui kategori aset'),
        });
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Kategori Aset" />
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <CardTitle>Edit Kategori Aset</CardTitle>
                                    <CardDescription>Perbarui informasi kategori aset</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
